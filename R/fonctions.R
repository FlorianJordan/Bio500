...
"
  ...
  fonction_requete_liens<-function(){
  sql_requete_liens <- "
SELECT etudiant1 as etudiant, count(etudiant2) as liens
FROM collaborations WHERE sigle NOT LIKE '%TSB303%'
GROUP BY etudiant
...
"
  ...
  fonction_requete_prog<-function(){
  sql_requete_prog <- "
SELECT nom_prenom,programme
FROM noeuds
"
  }
  fonction_figure_2<-function(){
